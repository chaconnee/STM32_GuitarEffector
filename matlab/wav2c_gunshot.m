%% wav2c_gunshot.m —— WAV 文件转 C 头文件 (gunshot 能量归一化方法)
%
% 功能: 读取 IR WAV 文件，重采样到 32kHz，截断，能量归一化，输出 .h 文件
% 归一化方法: gunshot (scale = 1/sqrt(sum(x^2)))
%
% 与 wav2c.m 的区别:
%   - wav2c.m: 峰值归一化 + 功率增益 (NeuralRack 方法)
%   - 本脚本: 能量归一化 (gunshot 方法), 保留原始动态
%
...
OUTPUT_NAME = 'ir_ac30_gunshot';   % 变量名和文件名 (注明 gunshot 方法)
...
%% ========== 3. 截断 + 能量归一化 (gunshot 方法) ==========
fprintf('\n[3/4] 截断 + 能量归一化 (gunshot 方法)\n');
...
% 计算总功率
sum_sq = sum(ir_used .* ir_used);
fprintf('      总功率: %.6f\n', sum_sq);

% 能量归一化: scale = 1 / sqrt(总功率)
scale = 1.0 / sqrt(sum_sq);
ir_used = ir_used * scale;
fprintf('      能量归一化: scale = %.6f\n', scale);
fprintf('      归一化后峰值: %.6f\n', max(abs(ir_used)));
...
%% ========== 可选: 绘制分析图 ==========
if PLOT_EN
    figure('Name', 'IR 分析 (gunshot 方法)', 'Position', [50 50 1200 500]);
    ...
    sgtitle(sprintf('WAV -> C 转换: %s (gunshot 方法)', OUTPUT_NAME), 'FontSize', 12);
end
