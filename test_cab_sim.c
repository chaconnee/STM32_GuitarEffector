...
void init() {
    ...
    // Create impulse IR
    cab_data.ir_fft[0] = 1.0f; // Impulse at index 0
...
int main() {
    ...
    float buf[128];
    for(int i=0; i<128; i++) buf[i] = (i == 0) ? 1.0f : 0.0f; // Impulse in block 1
    
    process(buf, buf);
    
    printf("Output block 1 (should have impulse at 0): %f, %f, %f\n", buf[0], buf[1], buf[2]);
    
    for(int i=0; i<128; i++) buf[i] = 0.0f; 
    process(buf, buf);
    printf("Output block 2 (should be zero): %f, %f, %f\n", buf[0], buf[1], buf[2]);
