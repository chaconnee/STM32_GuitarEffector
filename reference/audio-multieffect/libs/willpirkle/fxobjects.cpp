...
bool AudioFilter::calculateFilterCoeffs()
{
	...
	float fc = audioFilterParameters.fc;
	...
	if (algorithm == filterAlgorithm::kImpInvLP1)
	{
		...
		float omega = 2.0*kPi*fc;
		float eT = exp(-T*omega);
	...
	{
		float alpha = 2.0*kPi*fc / sampleRate;
		float p_Re = -alpha / (2.0*Q);
		float zeta = 1.0 / (2.0 * Q);
		float p_Im = alpha*pow((1.0 - (zeta*zeta)), 0.5);
		float c_Re = 0.0;
		float c_Im = alpha / (2.0*pow((1.0 - (zeta*zeta)), 0.5));

		float eP_re = exp(p_Re);
		coeffArray[a0] = c_Re;
	...
	else if (algorithm == filterAlgorithm::kMatchLP2A)
	{
		...
		float theta_c = 2.0*kPi*fc / sampleRate;

		float q = 1.0 / (2.0*Q);

		// --- impulse invariant
		float b_1 = 0.0;
		float b_2 = exp(-2.0*q*theta_c);
		if (q <= 1.0)
		{
		...
		// --- TIGHT FIT --- //
		float B0 = (1.0 + b_1 + b_2)*(1.0 + b_1 + b_2);
		float B1 = (1.0 - b_1 + b_2)*(1.0 - b_1 + b_2);
		float B2 = -4.0*b_2;

		float phi_0 = 1.0 - sin(theta_c / 2.0)*sin(theta_c / 2.0);
		float phi_1 = sin(theta_c / 2.0)*sin(theta_c / 2.0);
		float phi_2 = 4.0*phi_0*phi_1;

		float R1 = (B0*phi_0 + B1*phi_1 + B2*phi_2)*(Q*Q);
		float A0 = B0;
		float A1 = (R1 - A0*phi_0) / phi_1;
		...
		float a_0 = 0.5*(pow(A0, 0.5) + pow(A1, 0.5));
		float a_1 = pow(A0, 0.5) - a_0;
		float a_2 = 0.0;
	...
	else if (algorithm == filterAlgorithm::kMatchLP2B)
	{
		...
		// --- LOOSE FIT --- //
		float f0 = theta_c / kPi; // note f0 = fraction of pi, so that f0 = 1.0 = pi = Nyquist

		float r0 = 1.0 + b_1 + b_2;
		float denom = (1.0 - f0*f0)*(1.0 - f0*f0) + (f0*f0) / (Q*Q);
		denom = pow(denom, 0.5);
		float r1 = ((1.0 - b_1 + b_2)*f0*f0) / (denom);
	...
	else if (algorithm == filterAlgorithm::kMatchBP2A)
	{
		...
		float R1 = B0*phi_0 + B1*phi_1 + B2*phi_2;
		float R2 = -B0 + B1 + 4.0*(phi_0 - phi_1)*B2;

		float A2 = (R1 - R2*phi_1) / (4.0*phi_1*phi_1);
		float A1 = R2 + 4.0*(phi_1 - phi_0)*A2;

		float a_1 = -0.5*(pow(A1, 0.5));
		float a_0 = 0.5*(pow((A2 + (a_1*a_1)), 0.5) - a_1);
		float a_2 = -a_0 - a_1;
	...
	else if (algorithm == filterAlgorithm::kMatchBP2B)
	{
		...
		float r0 = (1.0 + b_1 + b_2) / (kPi*f0*Q);
		float denom = (1.0 - f0*f0)*(1.0 - f0*f0) + (f0*f0) / (Q*Q);
		denom = pow(denom, 0.5);

		float r1 = ((1.0 - b_1 + b_2)*(f0 / Q)) / (denom);

		float a_1 = -r1 / 2.0;
		float a_0 = (r0 - a_1) / 2.0;
		float a_2 = -a_0 - a_1;
	...
	else if (algorithm == filterAlgorithm::kLPF1P)
	{
		...
		float theta_c = 2.0*kPi*fc / sampleRate;
		float gamma = 2.0 - cos(theta_c);

		float filter_b1 = pow((gamma*gamma - 1.0), 0.5) - gamma;
		float filter_a0 = 1.0 + filter_b1;
	...
	else if (algorithm == filterAlgorithm::kLPF2)
	{
		...
		float theta_c = 2.0*kPi*fc / sampleRate;
		float d = 1.0 / Q;
		float betaNumerator = 1.0 - ((d / 2.0)*(sin(theta_c)));
		float betaDenominator = 1.0 + ((d / 2.0)*(sin(theta_c)));
	...
	//	float mag = getMagResponse(theta_c, coeffArray[a0], coeffArray[a1], coeffArray[a2], coeffArray[b1], coeffArray[b2]);
		// --- update on calculator
		biquad.setCoefficients(coeffArray);
	...
	{
		// --- see book for formulae
		float theta_c = kPi*fc / sampleRate;
	...
	else if (algorithm == filterAlgorithm::kButterBPF2)
	{
		...
		float theta_c = 2.0*kPi*fc / sampleRate;
		float BW = fc / Q;
		float delta_c = kPi*BW / sampleRate;
	...
	else if (algorithm == filterAlgorithm::kMMALPF2 || algorithm == filterAlgorithm::kMMALPF2B)
	{
		// --- see book for formulae
		float theta_c = 2.0*kPi*fc / sampleRate;
		float resonance_dB = 0;

		if (Q > 0.707)
		{
			float peak = Q*Q / pow(Q*Q - 0.25, 0.5);
			resonance_dB = 20.0*log10(peak);
		}

		// --- intermediate vars
		float resonance = (cos(theta_c) + (sin(theta_c) * sqrt(pow(10.0, (resonance_dB / 10.0)) - 1))) / ((pow(10.0, (resonance_dB / 20.0)) * sin(theta_c)) + 1);
		float g = pow(10.0, (-resonance_dB / 40.0));

		// --- kMMALPF2B disables the GR with increase in Q
		if (algorithm == filterAlgorithm::kMMALPF2B)
			g = 1.0;

		float filter_b1 = (-2.0) * resonance * cos(theta_c);
		float filter_b2 = resonance * resonance;
		float filter_a0 = g * (1 + filter_b1 + filter_b2);
	...
	else if (algorithm == filterAlgorithm::kLowShelf)
	{
		// --- see book for formulae
		float theta_c = 2.0*kPi*fc / sampleRate;
		float mu = pow(10.0, boostCut_dB / 20.0);

		float beta = 4.0 / (1.0 + mu);
		float delta = beta*tan(theta_c / 2.0);
	...
	else if (algorithm == filterAlgorithm::kCQParaEQ)
	{
		// --- see book for formulae
		float K = tan(kPi*fc / sampleRate);
		float Vo = pow(10.0, boostCut_dB / 20.0);
		bool bBoost = boostCut_dB >= 0 ? true : false;

		float d0 = 1.0 + (1.0 / Q)*K + K*K;
		float e0 = 1.0 + (1.0 / (Vo*Q))*K + K*K;
		float alpha = 1.0 + (Vo / Q)*K + K*K;
		float beta = 2.0*(K*K - 1.0);
		float gamma = 1.0 - (Vo / Q)*K + K*K;
		float delta = 1.0 - (1.0 / Q)*K + K*K;
		float eta = 1.0 - (1.0 / (Vo*Q))*K + K*K;
	...
	{
		// --- see book for formulae
		float omega_c = kPi*fc;
		float theta_c = kPi*fc / sampleRate;

		float k = omega_c / tan(theta_c);
		float denominator = k*k + omega_c*omega_c + 2.0*k*omega_c;
		float b1_Num = -2.0*k*k + 2.0*omega_c*omega_c;
		float b2_Num = -2.0*k*omega_c + k*k + omega_c*omega_c;
	...
	{
		// --- see book for formulae
		float alphaNumerator = tan((kPi*fc) / sampleRate) - 1.0;
		float alphaDenominator = tan((kPi*fc) / sampleRate) + 1.0;
	...
	else if (algorithm == filterAlgorithm::kAPF2)
	{
		...
		float theta_c = 2.0*kPi*fc / sampleRate;
		float BW = fc / Q;
		float argTan = kPi*BW / sampleRate;
		if (argTan >= 0.95*kPi / 2.0) argTan = 0.95*kPi / 2.0;

		float alphaNumerator = tan(argTan) - 1.0;
		float alphaDenominator = tan(argTan) + 1.0;
		float alpha = alphaNumerator / alphaDenominator;
	...
	else if (algorithm == filterAlgorithm::kResonA)
	{
		...
		float theta_c = 2.0*kPi*fc / sampleRate;
		float BW = fc / Q;
		float filter_b2 = exp(-2.0*kPi*(BW / sampleRate));
		float filter_b1 = ((-4.0*filter_b2) / (1.0 + filter_b2))*cos(theta_c);
		float filter_a0 = (1.0 - filter_b2)*pow((1.0 - (filter_b1*filter_b1) / (4.0 * filter_b2)), 0.5);
	...
	else if (algorithm == filterAlgorithm::kResonB)
	{
		...
		float filter_b2 = exp(-2.0*kPi*(BW / sampleRate));
		float filter_b1 = ((-4.0*filter_b2) / (1.0 + filter_b2))*cos(theta_c);
		float filter_a0 = 1.0 - pow(filter_b2, 0.5); // (1.0 - filter_b2)*pow((1.0 - (filter_b1*filter_b1) / (4.0 * filter_b2)), 0.5);
...
\returns the biquad processed output y(n)
*/
float AudioFilter::processAudioSample(float xn)
{
...
*/
void AudioDetector::setAttackTime(float attack_in_ms, bool forceCalc)
{
...
\param forceCalc flag to force a re-calculation of time constant even if values have not changed.
*/
void AudioDetector::setReleaseTime(float release_in_ms, bool forceCalc)
{
...
void FastFFT::initialize(unsigned int _frameLength, windowType _window)
{
	...
	windowBuffer = new float[frameLength];
	memset(&windowBuffer[0], 0, frameLength * sizeof(float));
...
*/
fftw_complex* FastFFT::doFFT(float* inputReal, float* inputImag)
{
...
\returns a pointer to a fftw_complex array: a 2D array of real (column 0) and imaginary (column 1) parts
*/
fftw_complex* FastFFT::doInverseFFT(float* inputReal, float* inputImag)
{
...
void PhaseVocoder::initialize(unsigned int _frameLength, unsigned int _hopSize, windowType _window)
{
	...
	overlap = hopSize > 0.0 ? 1.0 - (float)hopSize / (float)frameLength : 0.0;
	...
	inputBuffer = new float[frameLength];
	memset(&inputBuffer[0], 0, frameLength * sizeof(float));
	...
	outputBuffer = new float[frameLength * 4];
	memset(&outputBuffer[0], 0, (frameLength*4.0) * sizeof(float));
...
*/
float PhaseVocoder::processAudioSample(float input, bool& fftReady)
{
	...
	float currentOutput = outputBuffer[outputReadIndex];
	...
	// --- push into buffer
	inputBuffer[inputWriteIndex++] = (float)input;
...
*/
void PhaseVocoder::doOverlapAdd(float* outputData, int length)
{
